...
class ComputeDistance {
 public:
  ComputeDistance(int t_nearest_neighbor, MPI_Comm new_comm);
...
//  void Read(const string& file);
  void Read(char* tempDataInputBuffer);
  // The documents with index 3, 8, 13 ... are stored on me.

  // Compute the distance from my documents to all other documents.
  void ParallelComputeEuclidean();
  void ParallelComputeManhattan();
  void ParallelComputeCorrelation();
  void ParallelComputeCosine();
...
//  void Write(const string& file);
 ...
 private:
  ...
  double cityDistance(const Document& doc1, const Document& doc2);
  double calcSummation(const Document& doc1);
