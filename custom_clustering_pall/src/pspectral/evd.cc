...
void EigenSolverSymmetric::Solve(T& av,
  ...
  pdseupd_((long int*)communicator,
...
/*void Evd::Read(const string& filename) {
  ...
  MPI_Bcast(&num_total_rows_, 1, MPI_INT, 0, communicator);
...
void Evd::Read(char* dataInputBuffer) {
  // Master read the total number of rows.
  num_total_rows_ = 0;
  if (myid_ == 0) {
//    ifstream fin2(filename.c_str());
...
void Evd::operator()(double *x, double *y) const {
  ...
  MPI_Allgather(buff, row_count_max_, MPI_DOUBLE,
                buff2, row_count_max_, MPI_DOUBLE, communicator);
  int index = 0;
  for (int i = 0; i < pnum_; ++i) {
...
/*void Evd::Write(const string& eigenvaluefile,
	...
	std::cout<<"NO PROBLEM TILL HERE "<< pnum_ << " num_converged_= "<<num_converged_<<" eigenvaluefile.c_str() " << eigenvaluefile.c_str() <<std::endl;
...
char* Evd::Write(/*char* dataEigenvalue,*/ ) {

	char* dataEigenvector;
	std::stringstream /*dataBufferEigenvalue,*/ dataBufferEigenvector;

//	std::cout<<"NO PROBLEM TILL HERE "<< pnum_ << " num_converged_= "<<num_converged_<<std::endl;
...
//    for (int j = 0; j < num_converged_; ++j) {
////      fout << eigen_values_[j] << std::endl;
//    	dataBufferEigenvalue << eigen_values_[j] << std::endl;
//    }
////    fout.close();
//  }
...
//        fout.open(eigenvectorfile.c_str(), std::ios::app);
      ...
      for (int i = 0; i < num_local_rows_; ++i) {
        for (int j = 0; j < num_converged_; ++j) {
          if (j != 0) dataBufferEigenvector << " ";
//          fout << eigen_vectors_[j * num_local_rows_ + i];
          dataBufferEigenvector << eigen_vectors_[j * num_local_rows_ + i];
        }
//        fout << std::endl;
        dataBufferEigenvector << std::endl;
      }
//      fout.close();
    }
    MPI_Barrier(communicator);
  }
//  	 dataEigenvalue = new char[dataBufferEigenvalue.str().size()+1];
//     strcpy(dataEigenvalue, dataBufferEigenvalue.str().c_str());

     dataEigenvector = new char[dataBufferEigenvector.str().size()+1];
     strcpy(dataEigenvector, dataBufferEigenvector.str().c_str());
     return dataEigenvector;
}
...
using namespace std;
int _main_evd_(int argc, char** argv) {
  MPI_Init(&argc, &argv);
