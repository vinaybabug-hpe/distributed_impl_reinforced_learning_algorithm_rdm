...
DistanceToSimilarity::DistanceToSimilarity(MPI_Comm new_comm):communicator (new_comm) {
...
void DistanceToSimilarity::ReadAndSetSymmetric(char* dataInputBuffer) {
//  ifstream fin2(filename.c_str());
  ...
  for (int i = 0; i < num_total_docs_; ++i) {
    if (i % pnum_ == myid_) {
      ...
      for (int j = 0; j < sorted_index.size(); ++j) {
        ...
        std::cout << (*m.find(sorted_index[j])).first << ":" << (*m.find(sorted_index[j])).second;
      }
      fout << std::endl;
      std::cout << std::endl;
      fout.close();
...
char* DistanceToSimilarity::Write() {
...
//          std::cout << " ";
        ...
        dataBuffer << (*m.find(sorted_index[j])).first << ":" << (*m.find(sorted_index[j])).second;
//        std::cout << (*m.find(sorted_index[j])).first << ":" << (*m.find(sorted_index[j])).second;
      }
      dataBuffer << std::endl;
//      std::cout << std::endl;
...
//using namespace std;
//int _main_d2s_(int argc, char** argv) {
//  MPI_Init(&argc, &argv);
//  string FLAGS_input = "";
//  string FLAGS_output = "";
