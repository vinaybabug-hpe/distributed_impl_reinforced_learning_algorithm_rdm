...
KMeans::KMeans(MPI_Comm new_comm)
    : num_total_rows_(0),
      my_start_row_index_(0), communicator (new_comm) {
...
void KMeans::Read(char* dataInputBuffer) {
  num_total_rows_ = 0;
  if (myid_ == 0) {
//    ifstream fin2(filename.c_str());
...
//        fout.open(filename.c_str(), std::ios::app);
      ...
      for (int i = 0; i < local_memberships_.size(); ++i) {
    	  dataBuffer << local_memberships_[i] << std::endl;
      }
//      fout.close();
    }
    MPI_Barrier(communicator);
  ...
  if (kmeans_initialize_method == "orthogonal_centers") {
    ...
    for (int k = 1; k < num_clusters_; ++k) {
      ...
      MPI_Bcast(&cluster_centers_[k][0],
                num_columns_,
                MPI_DOUBLE,
                global_orthogonal_proc_index, communicator);
    }
    for (int i = 0; i < num_clusters_; ++i) {
...
using namespace std;
int _main_kmeans_(int argc, char** argv) {
  MPI_Init(&argc, &argv);
