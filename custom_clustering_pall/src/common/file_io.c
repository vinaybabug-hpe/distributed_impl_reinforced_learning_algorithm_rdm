                  ...
                  int  *numCoords)     /* no. coordinates */
{
  ...
  /* input file is in ASCII format -------------------------------*/
        ...
        rewind(infile);
        if (DEBUG) printf("lineLen = %d\n",lineLen);

        /* find the no. coordinates of each object */
        (*numCoords) = 1;
        while (fgets(line, lineLen, infile) != NULL) {
            if (strtok(line, " \t\n") != 0) {
                /* don't ignore the first coordiinate; */
                while (strtok(NULL, " ,\t\n") != NULL) (*numCoords)++;
...
/*---< file_read() >---------------------------------------------------------*/
void file_read_ascii1DMEX(
                  char *filename,      /* input file name */
                  int  *numObjs,       /* no. data objects (local) */
                  int  *numCoords, float *objects1d)     /* no. coordinates */
{
    float **objects;
  ...
  /* input file is in ASCII format -------------------------------*/
        ...
        while (fgets(line, lineLen, infile) != NULL) {
            ...
            for (j=1; j<(*numCoords); j++) {
                ...
                if (DEBUG && i == 0) /* print the first object */
        ...
        for(i=0;i<(*numObjs);i++){
        	for(j=0;j<(*numCoords);j++){
        		objects1d[j+i*(*numCoords)]=objects[i][j];
        	}
        }
...
//    float **objects;
  ...
  /* input file is in ASCII format -------------------------------*/
        ...
        while (fgets(line, lineLen, infile) != NULL) {
            if ((token=strtok(line, " \t\n")) == NULL) continue;
            objects1d[i*(*numCoords)+j] = atof(token);
            for (j=1; j<(*numCoords); j++) {
            	objects1d[i*(*numCoords)+j] = atof(strtok(NULL, " ,\t\n"));
                if (DEBUG && i == 0) /* print the first object */
                    printf("object[i=%d][j=%d]=%f\n",i,j,objects1d[i*(*numCoords)+j]);
            }
            i++;
        }
        assert(i == *numObjs);
...
/*---< file_read() >---------------------------------------------------------*/
/**
 * Same as file_read() but does not skip first column in file.
 */
...
//    printf("\n[file_io.c: Reading data from file %s.", filename);
...
/*---< file_read() >---------------------------------------------------------*/
/**
 * This function skips first column
 */
