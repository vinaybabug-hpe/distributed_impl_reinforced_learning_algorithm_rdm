...
static
double cosineDist(int n, double** data1, double** data2, int** mask1, int** mask2,
...
/* ********************************************************************* */
static
double mahalanobisDist(int n, double** data1, double** data2, int** mask1, int** mask2,
  const double weight[], int index1, int index2, int transpose)
  ...
  if (transpose==0) /* Calculate the distance between two rows */
  {
	  ...
	  for (i = 0; i < n; i++) {
			if (mask1[index1][i] && mask2[index2][i]) {
				...
				mean1 += data1[index1][i];
				mean2 += data2[index2][i];
			}
		}
	  mean1 /= n;
	  mean2 /= n;
		...
		for (i = 0; i < n; i++) {
			...
			if (mask1[index1][i] && mask2[index2][i]) {
				...
				covar = (data1[index1][i] - mean1)
						* (data2[index2][i] - mean2);
				xdiff = data1[index1][i] - data2[index2][i];
				result += weight[i] * (xdiff * covar * xdiff);
				tweight += weight[i];
			}
		}
	  ...
	  double mean1 = 0.;
		  ...
		  for (i = 0; i < n; i++) {
				if (mask1[index1][i] && mask2[index2][i]) {
					...
					mean1 +=  data1[i][index1];
					mean2 +=  data2[i][index2];
				}
			}
		  mean1 /= n;
		  mean2 /= n;
...
static
double jaccardDist(int n, double** data1, double** data2, int** mask1, int** mask2,
  const double weight[], int index1, int index2, int transpose)
  ...
  if (transpose==0) /* Calculate the distance between two rows */
  {
	 	for (i = 0; i < n; i++) {
			...
			if (mask1[index1][i] && mask2[index2][i]) {
				double term1 = data1[index1][i];
				double term2 = data2[index2][i];
				jaccard_similarity +=  weight[i] * ((term1<term2?term1:term2)/(term1>term2?term1:term2));
				tweight += weight[i];
			}
		}
...
/* ********************************************************************* */
static
double chebyshevDist(int n, double** data1, double** data2, int** mask1, int** mask2,
  const double weight[], int index1, int index2, int transpose)
...
{ double result = 0.;
 ...
 chevDist = malloc(n * sizeof(double));
  ...
  if (transpose==0) /* Calculate the distance between two rows */
  {
	 	for (i = 0; i < n; i++) {
			if (mask1[index1][i] && mask2[index2][i]) {
				double term1 = data1[index1][i];
				double term2 = data2[index2][i];
				chevDist[i] =  weight[i] * (term1>term2?term1:term2);
				tweight += weight[i];
			}
		}
...
/* ********************************************************************* */
static
double hammingDist (int n, double** data1, double** data2, int** mask1, int** mask2,
  const double weight[], int index1, int index2, int transpose)
...
{ double result = 0.;
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[index1][i] && mask2[index2][i])
      { double term = hamming_distance(data1[index1][i], data2[index2][i]);
  ...
  { for (i = 0; i < n; i++)
    { if (mask1[i][index1] && mask2[i][index2])
      { double term = hamming_distance(data1[i][index1] , data2[i][index2]);
        result += weight[i]*term*term;
        tweight += weight[i];
      }
    }
...
{ switch(dist)
  { case 'e': return &euclid;
    ...
    case 'o': return &cosineDist;
    case 'm': return &mahalanobisDist;
    case 'j': return &jaccardDist;
    case 'h': return &chebyshevDist;
    case 'g': return &hammingDist;
